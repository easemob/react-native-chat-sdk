...
- Remove `ChatRoomManager.createChatRoom` and `ChatRoomManager.destroyChatRoom`; create and destroy chat rooms through the server-side REST API. Remove `ChatRoomManager.getAllChatRooms` (removed by the native SDKs on both platforms).
