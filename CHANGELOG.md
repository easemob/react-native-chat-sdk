...
- Token login only: `ChatClient.loginWithToken` is the only remaining login method. Remove password login `ChatClient.login`, Agora token login `ChatClient.loginWithAgoraToken`, account registration `ChatClient.createAccount`, and `ChatClient.isLoginBefore`.
- Auto-login is removed: the `autoLogin` property of `ChatOptions` and the `onUserDidLoginFromOtherDevice` callback are removed; call `loginWithToken` proactively after app startup.
- Rename `ChatClient.renewAgoraToken` to `ChatClient.renewToken`.
...
- Fix event payload key mismatches between the native bridge and the TypeScript layer: `onAdminAdded`/`onAdminRemoved` in `ChatGroupEventListener` now correctly carry `admin`, `onAllGroupMemberMuteStateChanged`/`onAllChatRoomMemberMuteStateChanged` now correctly carry `isAllMuted`, `onRequestToJoinDeclined` on iOS now carries `decliner` (previously always `undefined` on iOS), and `ChatRecalledMessageInfo.recalledConvId` is now populated from recall events.
- `ChatMessage.isPeerRead`, `ChatMessage.isRead`, and `ChatMessage.groupReadReceiptCount` are now typed as `readonly` in TypeScript, matching the native read-only semantics.
