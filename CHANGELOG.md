...
- Login: password login, Agora token login, and account registration are removed from the client SDK. Only token login (`ChatClient.loginWithToken`) remains. Remove `ChatClient.login`, `ChatClient.loginWithAgoraToken`, `ChatClient.createAccount`, and `ChatClient.isLoginBefore`. Auto-login is removed: the `autoLogin` property of `ChatOptions` and the `onUserDidLoginFromOtherDevice` callback are removed; call `loginWithToken` proactively after app startup. Rename `ChatClient.renewAgoraToken` to `ChatClient.renewToken`. The `onAppActiveNumberReachLimit` callback is no longer fired on iOS (the native delegate was removed); it remains available on Android.
