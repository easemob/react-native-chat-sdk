...
import type { ChatDataSyncType } from './common/ChatOptions';
import type { ChatGroup } from './common/ChatGroup';
import type {
  ChatMessage,
  ChatMessagePinInfo,
...
import type { ChatMessageReadReceipt } from './common/ChatMessageReadReceipt';
...
export enum ChatMultiDeviceEvent {
  ...
  /**
   * If user A clears the unread message count of a conversation on device A1, this event is triggered on device A2.
   *
   * See {@link ChatManager.clearConversationUnreadMessageCount}.
   */
  CONVERSATION_UNREAD_MESSAGE_COUNT_CLEARED = 65,
  /**
   * If user A clears the unread message counts of all conversations on device A1, this event is triggered on device A2.
   *
   * See {@link ChatManager.clearAllConversationUnreadMessageCount}.
   */
  ALL_CONVERSATION_UNREAD_MESSAGE_COUNT_CLEARED = 66,
}
...
export function ChatMultiDeviceEventFromNumber(
  ...
  switch (params) {
    ...
    case 65:
      return ChatMultiDeviceEvent.CONVERSATION_UNREAD_MESSAGE_COUNT_CLEARED;
    case 66:
      return ChatMultiDeviceEvent.ALL_CONVERSATION_UNREAD_MESSAGE_COUNT_CLEARED;
...
export interface ChatConnectEventListener {
  ...
  /**
   * Occurs when the SDK starts to synchronize data of the specified type from the server.
   *
   * @param type The type of the data to be synchronized. It is a bitmask of {@link ChatDataSyncType}.
   */
  onDataSyncStart?(type: ChatDataSyncType): void;

  /**
   * Occurs when the SDK finishes synchronizing data of the specified type from the server.
   *
   * @param type The type of the synchronized data. It is a bitmask of {@link ChatDataSyncType}.
   * @param errorCode The error code of the synchronization result. `0` indicates success.
   */
  onDataSyncFinish?(type: ChatDataSyncType, errorCode: number): void;

  /**
   * Occurs when the local database is opened.
   *
   * The database is opened independently of the login process. SDK methods that depend on
   * the local database can be called only after this callback is triggered.
   *
   * @param username The user ID bound to the opened database.
 ...
 *
 * - If the recipient reads the received message, a read receipt will be returned to the sender (the read receipt function needs to be enabled: the message needs to be sent with {@link ChatMessage.isNeedReadReceipt} set to `true`).
 *
 ...
 *     }
 *     onMessageReadReceipts(receipts: ChatMessageReadReceipt[]): void {
 *       chatlog.log('ConnectScreen.onMessageReadReceipts', receipts);
 *     }
...
export interface ChatMessageEventListener {
   ...
   *
   * This callback is triggered in either of the following cases:
   *
   * - The recipient reads a message and sends a read receipt. See {@link ChatManager.sendMessageReadReceipts}.
   * - In a multi-device login scenario, the current user reads a message or clears the unread message count of a conversation on another device.
   *
   * @param receipts The message read receipt(s). See {@link ChatMessageReadReceipt}.
   */
