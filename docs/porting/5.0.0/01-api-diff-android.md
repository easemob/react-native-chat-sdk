...
| client_auto_login_removed | removed | `EMClient#init` 内基于 `getAutoLogin()`+`isLoggedInBefore()` 的自动登录逻辑 | 自动登录移除 | 行为变化，init 后需主动 `loginWithToken` |
...
| client_version_bump | new_property | `public final static String VERSION = "5.0.0"` | — | 原值 `"4.24.1"` |
...
| id | 类型 | Android 源签名 | 迁移文档依据 | 备注 |
|---|---|---|---|---|
| contact_get_all_from_server_removed | removed | `public List<String> getAllContactsFromServer() throws HyphenateException` | 服务端拉取 API 迁移 | 5.0.0 无任何从服务器拉好友列表的入口 |
...
| chat_fetch_conversations_from_server_removed | removed | `public Map<String, EMConversation> fetchConversationsFromServer() throws HyphenateException` + 3 个 async 重载：`asyncFetchConversationsFromServer(EMValueCallBack<Map<String, EMConversation>>)`、`asyncFetchConversationsFromServer(int pageNum, int pageSize, EMValueCallBack<Map<String, EMConversation>>)`、`asyncFetchConversationsFromServer(int limit, String cursor, EMValueCallBack<EMCursorResult<EMConversation>>)` | 服务端拉取 API 迁移 | 改 `getAllConversations()`（本地）+ `onDataSyncFinish(CONVERSATIONS)`；迁移文档称「全部 4 个 async 重载」，实际 diff 仅见 3 个，见疑点清单 |
...
| chat_report_message_removed | removed | `public void asyncReportMessage(String msgId, String tag, String reportReason, EMCallBack callback)` / `public void reportMessage(String msgId, String tag, String reportReason) throws HyphenateException` | 其他删除的 API | 举报提交至业务服务器 |
| chat_fetch_group_read_acks_removed | removed | `public EMCursorResult<EMGroupReadAck> fetchGroupReadAcks(String msgId, int pageSize, String startAckId) throws HyphenateException` / `public void asyncFetchGroupReadAcks(final String msgId, final int pageSize, final String startAckId, final EMValueCallBack<EMCursorResult<EMGroupReadAck>> callBack)` | 回执详情查询 | 由 `asyncFetchGroupMessageReadReceipts` 替代 |
...
| group_get_joined_groups_from_server_removed | removed | `public synchronized List<EMGroup> getJoinedGroupsFromServer()` / `public synchronized List<EMGroup> getJoinedGroupsFromServer(int pageIndex, int pageSize, boolean needMemberCount, boolean needRole)` / `public void asyncGetJoinedGroupsFromServer(final EMValueCallBack<List<EMGroup>> callback)` / `public void asyncGetJoinedGroupsFromServer(final int pageIndex, final int pageSize, boolean needMemberCount, boolean needRole, final EMValueCallBack<List<EMGroup>> callback)` | 服务端拉取 API 迁移 | 改 `getAllGroups()`（本地）+ `onDataSyncFinish(JOINED_GROUPS)` |
| group_get_public_groups_from_server_removed | removed | `public EMCursorResult<EMGroupInfo> getPublicGroupsFromServer(int pageSize, String cursor) throws HyphenateException` / `public void asyncGetPublicGroupsFromServer(final int pageSize, final String cursor, final EMValueCallBack<EMCursorResult<EMGroupInfo>> callback)` | 其他删除的 API | 群组目录由业务服务维护 |
...
| id | 类型 | Android 源签名 | 迁移文档依据 | 备注 |
|---|---|---|---|---|
| room_create_chat_room_removed | removed | `public EMChatRoom createChatRoom(String subject, String description, String welcomeMessage, int maxUserCount, List<String> members) throws HyphenateException` / `public void asyncCreateChatRoom(final String subject, final String description, final String welcomeMessage, final int maxUserCount, final List<String> members, final EMValueCallBack<EMChatRoom> callBack)` | 其他删除的 API | 服务端 REST 创建 |
| room_destroy_chat_room_removed | removed | `public void destroyChatRoom(String chatRoomId) throws HyphenateException` / `public void asyncDestroyChatRoom(final String chatRoomId, final EMCallBack callBack)` | 其他删除的 API | 服务端 REST 解散 |
...
1. **`onReadReceiptForGroupMessageUpdated` 不存在**：迁移文档「接收消息已读回执」「监听器回调变化汇总」两处称 `onReadAckForGroupMessageUpdated()` 改名为 `onReadReceiptForGroupMessageUpdated()`，但 `git grep` 在 SDK_5.0.0 全树（含内部包）中均无该方法——旧回调是直接删除，群消息回执状态更新实际由 `onMessageReadReceipts` 承载。RN 侧不应平版这个"新回调"。
2. **asyncFetchConversationsFromServer 重载数量不符**：迁移文档称删除「全部 4 个 asyncFetchConversationsFromServer(...) 重载」，SDK_4.24.1 源码中实际只有 3 个（无参 / pageNum+pageSize / limit+cursor）。若算上 `asyncFetchPinnedConversationsFromServer` 与 `asyncGetConversationsFromServerWithCursor` 则为 5 个相关接口。以源码为准：共删除 6 个服务端会话拉取相关方法。
3. **`EMClient#getDeviceInfo()` 疑似内部 API 暴露**：5.0.0 新增 `public JSONObject getDeviceInfo()`，组装 hid/os/os-version 供内部 `setPresence` 使用，无 javadoc、迁移文档未提及。建议 RN 侧不平版，待确认。
4. **`EMGroupConfigsType.toNativeMask(EnumSet)` / `EMDataSyncType.toNativeMask` 和 `fromNativeMask` 为 public static**：是跨 JNI 层的掩码转换工具，语义属内部但修饰符为 public。RN 侧无需平版。
...
7. **行为变化（无签名变化，RN 侧需注意）**：
   ...
   - `EMChatService` 旧保活逻辑移除（行为变化 6），`EMPushManager` Token 上传判断不再依赖 `isLoggedInBefore()`/`getAutoLogin()`（行为变化 7）。
   - 前后台检测改用 AndroidX `ProcessLifecycleOwner`（行为变化 5），SDK 全面迁移 AndroidX——对 RN 封装层无 API 影响，但影响 example 工程依赖。
8. **EMOptions 中 `AreaCode` 旧 int 常量全删**：`AREA_CODE_CN` 等 7 个 `public static final int` 一并删除，迁移文档只提了 `setAreaCode(int)` → `setAreaCode(AreaCode)`。若 RN 侧把 areaCode 作为 number 透传，需要改为枚举值映射。
...
> 说明：removed 方法数把同一名称的多个重载合并计为一条（如 `getJoinedGroupsFromServer` 4 个重载计 1 条）；若按单个方法签名计，删除的方法签名共 47 个。统计口径以清单表格行为准。
