...
| stream_chunk_sequence_number | renamed | 旧：`@property (nonatomic) long sequenceNumber;` → 新：`@property (nonatomic, readonly) BOOL isComplete;`（EMStreamChunk） | 同上 | 语义改变：不再暴露分片序号，改为是否完成 |
...
| id | 类型 | iOS 源签名 | 迁移文档依据 | 备注 |
| --- | --- | --- | --- | --- |
| error_contact_add_faild_typo | renamed | 旧：`EMErrorContactAddFaild = 1000,` → 新：`EMErrorContactAddFailed = 1000,` | 无 | 拼写修正，值不变；RN 侧若按名字映射需同步改名 |

## 3. 未匹配 / 疑点清单

1. **`EMGroup.users` 并非 5.0.0 新增**：迁移文档「主要新增 API」列出 `EMGroup#users`，但 `git grep` 确认 4.24.1 的 `EMGroup.h:212` 已存在 `@property (nonatomic, strong, readonly) NSArray<NSString *> *users;`，且 diff 未触及该属性。可能是语义/文档口径变化（按角色合并不去重），RN 侧若已封装 `users` 则无需改动。
...
8. **`EMErrorContactAddFaild → EMErrorContactAddFailed` 拼写修正未在迁移文档中提及**（值同为 1000）。RN 侧若用字符串名映射错误码需同步。
...
11. **`EMClientDelegate.h` 的 import 由 `EMCommonDefs.h` 改为 `EMOptions.h`**：因新回调参数使用 `EMDataSyncType`；对 RN 封装无直接影响。
...
15. **`EMConversation` 的 `conversationName`/`conversationAvatar` 是实例方法而非属性**（`- (NSString* _Nullable)conversationName;`），RN 封装时注意调用形式。
16. **4.24.2 侧分支hotfix（`EMUserInfoManager.mm` +2 行）不在 5.0.0 主线**：若 RN 当前基于 4.24.2 平版，需注意该 hotfix 内容在 5.0.0 中不存在（实现层，非公开 API）。
17. **`EMChatMessage+Private.h` +2 行**：内部头文件，未纳入清单；如 RN iOS 原生层直接引用了 Private 头需另行核对（正常不应引用）。
...
备注：removed 条数按「逻辑 API 条目」统计（同一方法的同步+异步两个重载合并在一行时按 1 条计，如 `client_register_with_username` 与 `client_register_with_username_completion` 分行计 2 条；group 同步删除系列每个方法 1 条）。若按 OC 方法签名逐个计数，实际删除的方法签名约 90+ 个。
