#!/usr/bin/env bash
# No-login device smoke test driver for Android. Shared by local runs and CI.
#
# Preconditions:
#   - a booted emulator/device reachable via adb;
#   - a debug APK built with API_SCRIPT=/data/local/tmp/rn_smoke_no_login.json
#     inlined at bundle time (the babel plugin in example/babel.config.js).
#
...
# assert every step was rejected with the expected error code.
#
...
OUT_LOG="$OUT_DIR/smoke-android-api_test.log"
