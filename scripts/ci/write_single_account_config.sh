...
# contains a real user token, so it is written with mode 0600 and must be
...
#   E2E_USER_TOKEN fresh user token for login (password login is gone in 5.0;
#                  fetch one via scripts/ci/fetch_e2e_user_token.js — tokens
#                  expire in ~24h, so they cannot be stored as CI secrets)
#
# Usage:
...
for var in E2E_APP_KEY E2E_USER_ID E2E_USER_TOKEN; do
  if [ -z "${!var:-}" ]; then
...
# explicit init/login sections. The token rides in the top-level `loginToken`
# key (see deriveLoginParams in auto_mode.ts), not in accounts[].mm.
OUT="$OUT" node -e '
const fs = require("fs");
const config = {
  appKey: [process.env.E2E_APP_KEY],
  accounts: [{ id: process.env.E2E_USER_ID }],
  loginToken: process.env.E2E_USER_TOKEN,
};
