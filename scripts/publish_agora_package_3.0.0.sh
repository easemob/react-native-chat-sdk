...
## 2026-01-16 新增部分
## 修改 typescript 源码
### 将 dohVendor 默认值从 1 修改为 2
if [ "${target_type}" == "agora" ]; then
  sed -i '' "s/this.dohVendor = params.dohVendor ?? 1;/this.dohVendor = params.dohVendor ?? 2;/g" "${output_dir}/${target_dir}/src/common/ChatOptions.ts"
fi
